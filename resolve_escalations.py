...
manager_agent.py) -- written whenever a StarDist image's automated retry loop exhausted its
iterations without the manager ever reaching accept=True.
...
For each pending escalation: shows you the final output image, then lets you talk directly to
the domain expert (the same ExpertReasoner used during training, still privately holding ground
truth) about what's wrong -- describe the issue or ask about a specific region, same as the
manager does during training, just driven by you instead of Qwen. When you're done (empty
line), the expert summarizes that conversation into transferable guidance and it gets folded
into the run's running_prompt (checkpoint.json) via merge_escalation_feedback, the same
...
from manager_agent import (
    EXPERT_PERSONA_COUNTGD, EXPERT_PERSONA_STARDIST, MODEL_ID, ExpertReasoner, QwenVLM,
    build_countgd_dossier, build_stardist_dossier, run_human_expert_dialogue,
)
...
def resolve_one(qwen: QwenVLM, record_path: Path, record: dict, checkpoint_path: Path) -> None:
    ...
    print(f"Last attempt: prob_thresh={last.get('prob_thresh')}, nms_thresh={last.get('nms_thresh')}, "
          f"predicted_count={last.get('predicted_count')}, feedback={last.get('feedback')!r}")
    ...
    conversation = run_human_expert_dialogue(expert, [record["final_image_path"]], input)
    if not conversation:
        print("No feedback given -- leaving this escalation pending.")
        return
