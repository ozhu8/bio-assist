    ...
    for i, task in enumerate(tasks, 1):
        ...
        if task["agent"] == "countgd":
            ...
            note = run_cellvit_trial(
                ...
                tissue=task["tissue"], expert_notes=expert_context, escalate=False,
            )
        else:
            note = run_stardist_trial(
                manager, task["image_path"], task["image_id"], task["ground_truth_labels"],
                args.max_iterations, output_dir, tissue=task["tissue"], expert_notes=expert_context, escalate=False,
            )
        notes.append(note)
